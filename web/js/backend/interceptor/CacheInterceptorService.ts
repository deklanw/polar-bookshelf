...
export class CacheInterceptorService {
    ...
    public async handleWithCache(
        ...
        log.debug(
            `Calling callback now for: ${request.url} (${
                buffer.byteLength
            } bytes)`
        );
    ...
    public async handleWithNetRequest(
        request: Request,
        callback: BufferCallback
    ) {
        ...
        const netRequest = net
            ...
            .on('response', async response => {
                ...
                const contentType = CacheInterceptorService.parseContentType(
                    response.headers['content-type']
                );

                log.debug(
                    `Using mimeType=${contentType.mimeType} for ${request.url}`
                );
    ...
    public async start() {
        ...
        await this.interceptBufferProtocol(
            'http',
