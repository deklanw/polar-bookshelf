...
describe('CacheEntriesFactory', function() {
    describe('Load CHTML', function() {
        ...
        beforeEach(function(done) {
            ...
            fs.writeFileSync(
                FilePaths.join(tmpdir, 'test-load.json'),
                JSON.stringify(data, null, '  ')
            );

            fs.writeFileSync(
                FilePaths.join(tmpdir, 'test-load.chtml'),
                '<html></html>'
            );
        ...
        it('createFromCHTML', async function() {
            const cacheEntriesHolder = await CacheEntriesFactory.createFromCHTML(
                path
            );
            ...
            assertJSON(
                Dictionaries.sorted(cacheEntriesHolder),
                Dictionaries.sorted(expected)
            );

            assert.equal(
                cacheEntriesHolder.metadata.url,
                'http://jakearchibald.com/2016/streams-ftw/'
            );
        });
    ...
    describe('Load PHZ', function() {
        it('createFromPHZ', async function() {
            ...
            const cacheEntriesHolder = await CacheEntriesFactory.createFromPHZ(
                path
            );
            ...
            assertJSON(cacheEntriesHolder.metadata, {
                ...
                browser: {
                    ...
                    description:
                        'Galaxy S8 mobile device running Chrome 61 but with width at 750',
                    userAgent:
                        'Mozilla/5.0 (Linux; Android 8.0.0; SM-G955U Build/R16NW) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Mobile Safari/537.36',
                    deviceEmulation: {
            ...
            assertJSON(
                Dictionaries.sorted(
                    cacheEntriesHolder.cacheEntries[
                        'https://journal.artfuldev.com/unit-testing-node-applications-with-typescript-using-mocha-and-chai-384ef05f32b2'
                    ]
                ),
                Dictionaries.sorted(expected)
