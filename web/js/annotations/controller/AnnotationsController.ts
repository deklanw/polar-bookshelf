...
export class AnnotationsController {
    public flashcardDialogWindow = new Nullable<DialogWindowClient>();

    public ipcClient = new Nullable<IPCClient<IPCEvent>>();
    ...
    private onMessageReceived(event: any) {
        ...
        if (data) {
            if (data.type === 'add-flashcard') {
                ...
                log.info(
                    'Creating flashcard from trigger event: ',
                    triggerEvent
                );

                this.createFlashcard(triggerEvent).catch(err =>
                    log.error('Could not create flashcard: ', err)
                );
            }
        }
    }
    ...
    private async sendAnnotationDescriptor(triggerEvent: TriggerEvent) {
        ...
        const annotationDescriptors = AnnotationTriggerEvents.getAnnotationDescriptors(
            triggerEvent
        );
        ...
        await this.ipcClient
            .get()
            .execute('/create-flashcard/api/create', annotationDescriptor);
    ...
    private async createDialogWindow(): Promise<DialogWindowClient> {
        const appPath = './apps/card-creator/index.html';

        const resource = new Resource(ResourceType.FILE, appPath);
        const options = new DialogWindowOptions(resource, 700, 900);
        options.show = false;
