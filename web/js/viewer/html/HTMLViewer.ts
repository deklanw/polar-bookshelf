...
export class HTMLViewer extends Viewer {
    ...
    public start() {
        ...
        this.contentParent = <HTMLElement>(
            document.querySelector('#content-parent')
        );
        ...
        $(document).ready(async () => {
            ...
            this.frameResizer = new FrameResizer(
                this.contentParent,
                this.content
            );
            ...
            new IFrameWatcher(this.content, () => {
                ...
                const backgroundFrameResizer = new BackgroundFrameResizer(
                    this.contentParent,
                    this.content
                );
            ...
            window.addEventListener('resize', () => {
                this.doZoom();
                this.frameResizer!.resize(true).catch(err =>
                    log.error('Unable to resize: ', err)
                );
            });
    ...
    private _configurePageWidth() {
        ...
        log.info(
            `Configuring page with width=${docDimensions.width} and minHeight=${
                docDimensions.minHeight
            }`
        );

        document
            .querySelectorAll('#content-parent, .page, iframe')
            .forEach(element => {
    ...
    private _changeScaleMeta(scale: number) {
        const metaElement = notNull(
            document.querySelector("meta[name='polar-scale']")
        );
    ...
    private _removeAnnotations() {
        ...
        document
            .querySelectorAll('.page .annotation')
            .forEach(function(annotation) {
                (annotation.parentElement as HTMLElement).removeChild(
                    annotation
                );
            });
    }
    ...
    private _loadRequestData() {
        ...
        if (ENABLE_VIDEO && file.indexOf('youtube.com/') !== -1) {
            ...
            this.content.contentWindow!.history.pushState(
                { html: embedHTML, pageTitle: 'Youtube Embed' },
                '',
                file
            );
        } else {
