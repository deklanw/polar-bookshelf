...
export class FrameResizer {
    ...
    public async resize(force: boolean = false) {
        ...
        const height = Styles.parsePX(
            Optional.of(this.host.style.height)
    ...
    private async getDocumentHeightForIFrame(
        iframe: HTMLIFrameElement
    ): Promise<Optional<number>> {
        return Optional.of(Documents.height(iframe.contentDocument));
    }

    private async getDocumentHeightForWebview(
        webview: Electron.WebviewTag
    ): Promise<Optional<number>> {
        const webContents = webview.getWebContents();

        const script = Functions.functionToScript(Documents.height);
        const height: number | undefined = await webContents.executeJavaScript(
            script
        );
