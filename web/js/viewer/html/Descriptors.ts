...
export class Descriptors {
    public static calculateDocDimensions(
        ...
        if (scrollBox.isPresent() && scrollBox.get().width > width) {
            ...
            if (
                !isPresent(scrollBox.get().widthOverflow) ||
    ...
    public static computeScrollBox(
        descriptor: PHZMetadata
    ): Optional<ScrollBox> {
        return this.computeScrollBoxFromBoxes(
            descriptor.scrollBox,
            descriptor.scroll
        );
    }

    public static computeScrollBoxFromBoxes(
        scrollBox?: ScrollBox,
        scroll?: ScrollBox
    ): Optional<ScrollBox> {
        return [Optional.of(scrollBox), Optional.of(scroll)]
            ...
            .filter(current =>
                current
                    .map(scrollBox => this.isScrollBox(scrollBox))
                    .getOrElse(false)
            )
    ...
    public static isScrollBox(scrollBox: ScrollBox) {
        return (
            typeof scrollBox.width === 'number' &&
            typeof scrollBox.height === 'number'
        );
    }
}
