import { MediaContents } from './MediaContents';
import { assertJSON } from '../../../../test/Assertions';

describe('MediaContents', function() {
    describe('parse', function() {
        it('basic', async function() {
            const content =
                'What is the capital of California? <img src="data:image/gif;base64,">';

            const expected = {
                content:
                    'What is the capital of California? <img src="1hqKtrsGiLerE69bmb5i.gif">',
                mediaFiles: [
                    {
                        filename: '1hqKtrsGiLerE69bmb5i.gif',
                        data:
                            '',
                    },
                ],
            };

            assertJSON(MediaContents.parse(content), expected);
        });
    });

    describe('toMediaFile', function() {
        it('basic', async function() {
            const dataURL =
                'data:image/gif;base64,';

            const expected: any = {
                filename: '1hqKtrsGiLerE69bmb5i.gif',
                data:
                    '',
            };

            assertJSON(MediaContents.toMediaFile(dataURL).get(), expected);
        });
    });
});
