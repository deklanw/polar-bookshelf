...
export class AnkiSyncEngine implements SyncEngine {
    ...
    public async sync(
        ...
        deckNameStrategy: DeckNameStrategy = 'default'
    ): Promise<PendingSyncJob> {
        // determine how to connect to Anki
        await AnkiConnectFetch.initialize();

        const noteDescriptors = await this.toNoteDescriptors(
            deckNameStrategy,
            docMetaSupplierCollection
        );

        const deckNames = Sets.toSet(
            noteDescriptors.map(noteDescriptor => noteDescriptor.deckName)
        );

        const deckDescriptors: DeckDescriptor[] = Array.from(deckNames).map(
            deckName => {
                return { name: deckName };
            }
        );

        return new PendingAnkiSyncJob(
            progress,
            deckDescriptors,
            noteDescriptors
        );
    ...
    protected async toNoteDescriptors(
        deckNameStrategy: DeckNameStrategy,
        docMetaSupplierCollection: DocMetaSupplierCollection
    ): Promise<NoteDescriptor[]> {
        const flashcardDescriptors = await FlashcardDescriptors.toFlashcardDescriptors(
            docMetaSupplierCollection
        );

        return flashcardDescriptors.map(flashcardDescriptor => {
            const deckName = this.computeDeckName(
                deckNameStrategy,
                flashcardDescriptor.docMeta.docInfo
            );

            const fields: { [name: string]: string } = {};

            // need to create the fields 'front' and 'back'
            Dictionaries.forDict(
                flashcardDescriptor.flashcard.fields,
                (key, value) => {
                    fields[key] = Optional.of(
                        value.HTML || value.TEXT || value.MARKDOWN
                    ).getOrElse('');
                }
            );

            const docInfoTags = Optional.of(
                flashcardDescriptor.docMeta.docInfo.tags
            );

            const tags = docInfoTags
            ...
            const modelName = FlashcardDescriptors.toModelName(
                flashcardDescriptor
            );
