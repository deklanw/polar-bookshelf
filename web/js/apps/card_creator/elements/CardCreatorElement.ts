...
export class CardCreatorElement extends HTMLElement {
    constructor() {
        ...
        const mainElement = shadowRoot.appendChild(
            CardCreatorElement.createElementHTML(template)
        );

        const schemaFormElement = <HTMLElement>(
            shadowRoot.getElementById('schema-form')
        );

        const inputController = new CreateFlashcardForm(schemaFormElement);
     ...
     */
    public static createElementHTML(innerHTML: string): HTMLElement {
