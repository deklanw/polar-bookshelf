...
export class PHZLoader extends FileLoader {
    ...
    public async registerForLoad(path: string): Promise<LoadedFile> {
        ...
        const cachedRequestsHolder = await this.cacheRegistry.registerFile(
            path
        );

        log.info(
            'cachedRequestsHolder: ' + JSON.stringify(cachedRequestsHolder)
        );
        ...
        const cachedRequest =
            cachedRequestsHolder.cachedRequests[
                cachedRequestsHolder.metadata.url
            ];
        ...
        const appPath = ResourcePaths.resourceURLFromRelativeURL(
        ...
        const queryData = `?file=${encodeURIComponent(
            cachedRequest.url
        )}&fingerprint=${fingerprint}&descriptor=${encodeURIComponent(
            descriptorJSON
