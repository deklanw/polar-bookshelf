...
export class MainAppController {
    ...
    constructor(fileLoader: FileLoader, webserver: Webserver) {
    ...
    public async cmdCaptureWebPage() {
        ...
        browserWindowOptions.width = browserWindowOptions.width! * 0.9;
        browserWindowOptions.height = browserWindowOptions.height! * 0.9;
        browserWindowOptions.center = true;

        const url = ResourcePaths.resourceURLFromRelativeURL(
            './apps/capture/start-capture/index.html'
        );
        ...
        if (Platforms.get() !== Platform.WINDOWS) {
            ...
            for (const browserWindow of browserWindows) {
                ...
                if (
                    ...
                    log.info(
                        `Skipping close window (not closeable) id=${id}, url=${url}`
                    );
                }
            }
        }
     ...
     */
    public async handleLoadDoc(
        path: string,
        newWindow: boolean = true
    ): Promise<BrowserWindow> {
        const extraTags = { type: 'viewer' };

        const browserWindowTag = {
            name: 'viewer',
            value: Hashcodes.createID(path),
        };

        return await SingletonBrowserWindow.getInstance(
            browserWindowTag,
            async () => {
                let window;

                if (newWindow) {
                    window = await MainAppBrowserWindowFactory.createWindow(
                        BROWSER_WINDOW_OPTIONS,
                        'about:blank'
                    );
                } else {
    ...
    public activateMainWindow() {
        ...
        browserWindows = browserWindows.filter(browserWindow =>
            browserWindow.isVisible()
        );

        if (browserWindows.length === 0) {
            AppLauncher.launchRepositoryApp().catch(err =>
                log.error('Unable to open repository app: ', err)
            );
            ...
            dialog.showOpenDialog(
                {
                    ...
                    filters: [{ name: 'Docs', extensions: ['pdf', 'phz'] }],
