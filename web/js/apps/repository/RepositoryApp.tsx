...
export class RepositoryApp {
    ...
    constructor() {
        this.repoDocInfoManager = new RepoDocMetaManager(
            this.persistenceLayerManager
        );
        this.repoDocInfoLoader = new RepoDocMetaLoader(
            this.persistenceLayerManager
        );
    }

    public async start() {
        const updatedDocInfoEventDispatcher: IEventDispatcher<
            IDocInfo
        > = new SimpleReactor();

        const syncBarProgress: IEventDispatcher<
            SyncBarProgress
        > = new SimpleReactor();
        ...
        this.persistenceLayerManager.addEventListener(event => {
            if (event.state === 'changed') {
                event.persistenceLayer.addEventListener(
                    (persistenceLayerEvent: PersistenceLayerEvent) => {
            ...
            <div>
                ...
                <HashRouter hashType="noslash">
                    <Switch>
                        <Route
                            exact
                            path="/(logout|overview|login|configured|invite)?"
                            render={renderDocRepoApp}
                        />
                        <Route
                            exact
                            path="/annotations"
                            render={renderAnnotationRepoApp}
                        />
                        <Route
                            exact
                            path="/whats-new"
                            render={renderWhatsNew}
                        />
                        <Route
                            exact
                            path="/community"
        ...
        this.repoDocInfoLoader.addEventListener(event => {
            for (const mutation of event.mutations) {
                if (
                    ...
                    this.repoDocInfoManager.updateFromRepoDocMeta(
                        mutation.fingerprint,
                        mutation.repoDocMeta!
                    );
                } else {
                    this.repoDocInfoManager.updateFromRepoDocMeta(
                        mutation.fingerprint
    ...
    private onUpdatedDocInfo(docInfo: IDocInfo): void {
        const handleUpdatedDocInfo = async () => {
            ...
            const docMeta = await this.persistenceLayerManager
                .get()
                .getDocMeta(docInfo.fingerprint);

            const repoDocMeta = RepoDocMetas.convert(
                docInfo.fingerprint,
                docMeta
            );
            ...
            if (RepoDocMetas.isValid(repoDocMeta)) {
                ...
                if (PersistenceLayerTypes.get() === 'cloud') {
                    ...
                    handleWriteDocMeta().catch(err =>
                        log.error('Unable to write docMeta to datastore: ', err)
                    );
                }
            } else {
                log.warn(
                    'We were given an invalid DocInfo which yielded a broken RepoDocMeta: ',
                    docInfo,
                    repoDocMeta
                );
            }
        ...
        handleUpdatedDocInfo().catch(err =>
            log.error('Unable to update doc info: ', err)
        );
    }
}
