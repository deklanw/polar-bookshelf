...
export class PDFImporter {
    ...
    public async importFile(filePath: string): Promise<Optional<ImportedFile>> {
        ...
        if (
            await PDFImporter.isWithinStashdir(directories.stashDir, filePath)
        ) {
        ...
        if (await persistenceLayer.contains(pdfMeta.fingerprint)) {
            log.warn(
                `This file is already present in the datastore with fingerprint ${
                    pdfMeta.fingerprint
                }: ${filePath}`
            );
            return Optional.empty();
        ...
        const filename =
            `${fileHashMeta.hashPrefix}-` +
            DatastoreFiles.sanitizeFileName(basename);
        ...
        const docMeta = DocMetas.create(
            pdfMeta.fingerprint,
            pdfMeta.nrPages,
            filename
        );

        docMeta.docInfo.title = Optional.of(pdfMeta.title).getOrElse(
            defaultTitle
        );
    ...
    private static async computeHashPrefix(
        path: string
    ): Promise<FileHashMeta> {
        const hashcode = await Hashcodes.createFromStream(
            Files.createReadStream(path)
        );
    ...
    private static async isWithinStashdir(
        stashDir: string,
        path: string
    ): Promise<boolean> {
        const currentDirname = await Files.realpathAsync(
            FilePaths.dirname(path)
        );
