...
export class FirebaseRunner {
    ...
    public async run(testingFunction: () => Promise<void>) {
        ...
        window.addEventListener('load', async () => {
            ...
            this.init().catch(err => log.error('Caught error on init', err));
        });
    }
    ...
    public async onAuth(user: firebase.User | null) {
        ...
        if (user) {
            ...
            const accountDetailsElement = document.getElementById(
                'account-details'
            );

            if (isPresent(accountDetailsElement)) {
                accountDetailsElement!.innerText = JSON.stringify(
