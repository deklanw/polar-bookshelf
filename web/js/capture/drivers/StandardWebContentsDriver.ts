...
import {
    WebContentsDriver,
    WebContentsEvent,
    WebContentsEventName,
} from './WebContentsDriver';
...
export class StandardWebContentsDriver implements WebContentsDriver {
    ...
    public async destroy() {
        ...
        Optional.of(this.browserWindow).map(browserWindow => {
    ...
    public async loadURL(
        url: string,
        waitForFinishLoad: boolean = true
    ): Promise<void> {
        const opts = {
        ...
        const result = WebContentsPromises.once(
            this.webContents!
        ).didFinishLoad();
    ...
    protected async initWebContents(
    ...
    private async initWebContentsEvents(webContents: WebContents) {
        const configurePDFDownloadHandler = (): void => {
            ...
            const willDownloadHandler = (
                ...
                ToasterMessages.send({
                    ...
                    message: 'PDF download starting for ' + basename,
                });
                ...
                downloadItem.on('updated', () => {
                    const progress = progressTracker.abs(
        ...
        webContents.on('dom-ready', e => {
            ...
            StandardWebContentsDriver.configureWebContents(
                webContents,
                this.browserProfile
            ).catch((err: Error) =>
                log.error('Could not configure web contents: ', err)
            );
        });
        ...
        webContents.on(
            'did-fail-load',
            (event, errorCode, errorDescription, validateURL, isMainFrame) => {
                log.info(
                    'did-fail-load: ',
                    {
                        event,
    ...
    public static async configureWebContents(
        webContents: WebContents,
        ...
        const configureBrowserScript = Functions.functionToScript(
            configureBrowser,
            windowDimensions
        );
