...
export abstract class AbstractWebviewWebContentsDriver extends StandardWebContentsDriver {
    ...
    protected async waitForWebview(): Promise<WebContents> {
        return new Promise<WebContents>(resolve => {
            this.browserWindow!.webContents.once(
                'did-attach-webview',
                (event, newWebContents: WebContents) => {
    ...
    protected async doInit() {
        this.browserWindowOptions = BrowserWindows.toBrowserWindowOptions(
            this.browserProfile
        );
        ...
        this.browserView = new DelegatedBrowserView([
            hostBrowserView,
            guestBrowserView,
        ]);

        WebContentsNotifier.on(
        ...
        WebContentsNotifier.on(
            ...
            (event: MainIPCEvent<Browser>) => {
                ...
                this.browserProfile = BrowserProfiles.toBrowserProfile(
                    browser,
                ...
                this.browserView!.configure(this.browserProfile).catch(
                    (err: Error) => log.error('Unable to configure: ', err)
                );
            }
        );
    ...
    protected async handleConfigureWindow() {
        ...
        await StandardWebContentsDriver.configureWebContents(
            this.webContents!,
            this.browserProfile
        );
    ...
    protected async doInitWebview() {
        ...
        const resourceURL = ResourcePaths.resourceURLFromRelativeURL(
            this.appPath
        );
    ...
    protected computeHostBrowserWindowOptions() {
        ...
        const browserWindowOptions = BrowserWindows.toBrowserWindowOptions(
            this.browserProfile
        );
        ...
        if (this.browserProfile.hosted) {
            ...
            browserWindowOptions.width =
                notNull(browserWindowOptions.width) * 1.35;
        }

        browserWindowOptions.height = Math.round(
            notNull(browserWindowOptions.width) * (11 / 8.5)
        );
    ...
    private async doInitGuestWebviewDimensions() {
        ...
        function setWebviewDimensions(
            ...
            const querySelector = <HTMLElement>(
    ...
    public async configure(browserProfile: BrowserProfile): Promise<void> {
        for (const delegate of this.delegates) {
            delegate
                ...
                .catch(err =>
                    log.error(
                        'Unable to configure for browser profile: ',
                        browserProfile
                    )
                );
        }
...
class HostBrowserView implements BrowserView {
    ...
    constructor(
        driver: AbstractWebviewWebContentsDriver,
    ...
    private async doInitGuestWebviewDimensions() {
        ...
        const script = Functions.functionToScript(
            setWebviewDimensions,
            this.browserWindowOptions
        );
