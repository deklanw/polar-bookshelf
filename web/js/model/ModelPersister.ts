    ...
    constructor(
        ...
        const batcher = new Batcher(async () => {
            await this.persistenceLayer.write(
                this.docMeta.docInfo.fingerprint,
                this.docMeta
            );
            ++this.nrWrites;
        ...
        this.docMeta = Proxies.create(
            ...
            (traceEvent: TraceEvent) => {
                ...
                log.info(
                    `sync of persistence layer at ${traceEvent.path} : ${
                        traceEvent.property
                    }"`
                );
                ...
                setTimeout(() => {
                    batcher
                        ...
                        .run()
                        .catch(err =>
                            log.error('Unable to commit to disk: ', err)
                        );
                }, 0);
        ...
        this.persistenceLayer.addEventListenerForDoc(
            this.docMeta.docInfo.fingerprint,
            event => {
                ...
                if (
                    this.docMeta.docInfo.fingerprint !==
                    event.docInfo.fingerprint
                ) {
                    const detail = `${this.docMeta.docInfo.fingerprint} vs ${
                        event.docInfo.fingerprint
                    }`;
                    throw new Error(
                        `Attempt to update incorrect fingerprint: ` + detail
                    );
                }
