...
export class ControlledAnnotationBars {
    public static create(
        controlledPopupProps: ControlledPopupProps,
    ...
    private static registerEventListener(
        ...
        ActiveSelections.addEventListener(activeSelectionEvent => {
            ...
            switch (activeSelectionEvent.type) {
                case 'created':
                    annotationBar = this.createAnnotationBar(
                        pageNum,
                        pageElement,
                        annotationBarCallbacks,
                        activeSelectionEvent
    ...
    private static computePosition(
        ...
        let origin: Point = Optional.of(pageElement.getBoundingClientRect())
            .map(rect => {
        ...
        const relativePoint: Point = Points.relativeTo(origin, point);
    ...
    private static createAnnotationBar(
        pageNum: number,
        pageElement: HTMLElement,
        annotationBarCallbacks: AnnotationBarCallbacks,
        ...
        const point: Point = {
            x:
                activeSelectionEvent.boundingClientRect.left +
                activeSelectionEvent.boundingClientRect.width / 2,
        ...
        annotationBar.addEventListener('mouseup', event =>
            event.stopPropagation()
        );
        annotationBar.addEventListener('mousedown', event =>
            event.stopPropagation()
        );

        const style = `position: absolute; top: ${position.y}px; left: ${
            position.x
        }px; z-index: 9999;`;
