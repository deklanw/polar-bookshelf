...
export class AnnotationBarService {
    ...
    public start(): void {
        ...
        document.body.addEventListener('click', event => {
            if (event.target instanceof HTMLElement) {
                const annotationType = event.target.getAttribute(
    ...
    private onDocumentLoaded(event: DocumentLoadedEvent) {
        ...
        CommentPopupBars.create(
            commentBarControlledPopupProps,
            commentPopupBarCallbacks
        );
        ...
        const triggerPopupEventDispatcher = new SimpleReactor<
            TriggerPopupEvent
        >();
            ...
            commentBarControlledPopupProps.triggerPopupEventDispatcher.dispatchEvent(
                {
                    point: {
                        x:
                            activeSelection.boundingClientRect.left +
                            activeSelection.boundingClientRect.width / 2,
        ...
        const onHighlighted: OnHighlightedCallback = (
            highlightCreatedEvent: HighlightCreatedEvent
        ) => {
            RendererAnalytics.event({
                category: 'annotations',
                action: 'text-highlight-created-via-annotation-bar',
            });
        ...
        ControlledAnnotationBars.create(
            annotationBarControlledPopupProps,
            annotationBarCallbacks
        );
    }
}
