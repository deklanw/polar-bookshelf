...
export class CommentsController {
    ...
    private readonly commentEventDispatcher = new SimpleReactor<
        CommentInputEvent
    >();
    ...
    private onMessageReceived(event: any) {
        ...
        if (data) {
            if (data.type === 'add-comment') {
                ...
                log.debug(
                    'Creating comment from trigger event: ',
                    triggerEvent
                );
                ...
                const annotationDescriptors = Object.values(
                    triggerEvent.matchingSelectors
                )
                ...
                if (annotationDescriptors.length === 1) {
                    ...
                    this.triggerCreateCommentPopup(
                        triggerEvent,
                        annotationDescriptor
                    ).catch(err =>
                        log.error('Could not create comment: ', err)
                    );
                } else {
    ...
    private async triggerCreateCommentPopup(
