...
export class AnnotationControlBar extends React.Component<IProps, IState> {
    ...
    public render() {
        ...
        return (
            <div className="annotation-control-bar">
                ...
                >
                    ...
                    <div style={Styles.barChild} className="flexbar-right">
                        ...
                        <Button
                            ...
                            style={Styles.button}
                            onClick={() =>
                                this.toggleActiveInputComponent('comment')
                            }
                        >
                            <CommentIcon />
                        </Button>
                        ...
                        <Button
                            ...
                            color="light"
                            onClick={() =>
                                this.toggleActiveInputComponent('flashcard')
                            }
                        >
                            <FlashcardIcon />
                        </Button>
                        ...
                        <AnnotationDropdown
                            ...
                            onJumpToContext={() =>
                                this.onJumpToContext(annotation)
                            }
                        />
                    </div>
                </div>
                ...
                <Collapse
                    timeout={0}
                    isOpen={this.state.activeInputComponent === 'comment'}
                >
                ...
                <Collapse
                    timeout={0}
                    isOpen={this.state.activeInputComponent === 'flashcard'}
                >
                    <AnnotationFlashcardBox
                        id={annotation.id}
                        onCancel={() => this.toggleActiveInputComponent('none')}
                        onFlashcardCreated={(type, fields) =>
                            this.onFlashcardCreated(type, fields)
                        }
                    />
                </Collapse>
            </div>
    ...
    private onJumpToContext(annotation: DocAnnotation) {
        AnnotationSidebars.scrollToAnnotation(
            annotation.id,
            annotation.pageNum
        );
    }

    private toggleActiveInputComponent(
        activeInputComponent: ActiveInputComponent
    ) {
        this.setState({
            activeInputComponent:
                this.state.activeInputComponent === activeInputComponent
                    ? 'none'
                    : activeInputComponent,
        });
    }
    ...
    private onFlashcardCreated(
        type: FlashcardType,
        fields: FrontAndBackFields | ClozeFields
    ) {
        RendererAnalytics.event({
            category: 'annotations',
            action: 'flashcard-created',
        });
        ...
        Functions.withTimeout(() => {
            ...
            if (flashcard) {
                annotation.pageMeta.flashcards[
                    flashcard.id
                ] = Flashcards.createMutable(flashcard);
            }
        });
    }
}
