...
export class DocAnnotations {
    ...
    public static createFromFlashcard(
        flashcard: Flashcard,
        pageMeta: PageMeta
    ): DocAnnotation {
        return {
    ...
    public static createFromComment(
        comment: Comment,
        pageMeta: PageMeta
    ): DocAnnotation {
        return {
    ...
    public static createFromAreaHighlight(
        ...
        return {
            ...
            pageNum: pageMeta.pageInfo.num,
            position: {
                x: this.firstRect(areaHighlight)
                    .map(current => current.left)
                    .getOrElse(0),
                y: this.firstRect(areaHighlight)
                    .map(current => current.top)
                    .getOrElse(0),
            },
    ...
    public static createFromTextHighlight(
        ...
        if (
            isPresent(textHighlight.text) &&
            typeof textHighlight.text === 'object'
        ) {
            // TODO: move this to an isInstanceOf in Texts
            if (
                'TEXT' in <any>textHighlight.text ||
                'HTML' in <any>textHighlight.text
            ) {
        ...
        return {
            ...
            pageNum: pageMeta.pageInfo.num,
            position: {
                x: this.firstRect(textHighlight)
                    .map(current => current.left)
                    .getOrElse(0),
                y: this.firstRect(textHighlight)
                    .map(current => current.top)
                    .getOrElse(0),
            },
