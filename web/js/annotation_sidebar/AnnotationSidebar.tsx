...
> {
    ...
    constructor(props: AnnotationSidebarProps, context: any) {
        ...
        this.docAnnotationIndex = DocAnnotationIndexes.rebuild(
    ...
    public componentDidMount(): void {
        ...
        new AreaHighlightModel().registerListener(
            this.props.docMeta,
            annotationEvent => {
                const docAnnotation = this.convertAnnotation(
                    annotationEvent.value,
                    annotationValue =>
                        DocAnnotations.createFromAreaHighlight(
                            annotationValue,
                            annotationEvent.pageMeta
                        )
                );

                this.handleAnnotationEvent(
                    annotationEvent.id,
        ...
        new TextHighlightModel().registerListener(
            this.props.docMeta,
            annotationEvent => {
                const docAnnotation = this.convertAnnotation(
                    annotationEvent.value,
                    annotationValue =>
                        DocAnnotations.createFromTextHighlight(
                            annotationValue,
                            annotationEvent.pageMeta
                        )
                );
        ...
        new CommentModel().registerListener(
            this.props.docMeta,
            annotationEvent => {
                const comment: Comment =
                    annotationEvent.value || annotationEvent.previousValue;
                const childDocAnnotation = DocAnnotations.createFromComment(
                    comment,
                    annotationEvent.pageMeta
                );

                this.handleChildAnnotationEvent(
                    annotationEvent.id,
                    annotationEvent.traceEvent.mutationType,
                    childDocAnnotation
                );
        ...
        new FlashcardModel().registerListener(
            this.props.docMeta,
            annotationEvent => {
                const flashcard: Flashcard =
                    annotationEvent.value || annotationEvent.previousValue;
                const childDocAnnotation = DocAnnotations.createFromFlashcard(
                    flashcard,
                    annotationEvent.pageMeta
                );
    ...
    private convertAnnotation<T>(
        value: any | undefined | null,
        converter: (input: any) => T
    ) {
        if (!isPresent(value)) {
            return undefined;
        }
    ...
    private handleChildAnnotationEvent(
        id: string,
        ...
        if (mutationType !== MutationType.DELETE) {
            ...
            annotation.children.sort(
                (c0, c1) => -c0.created.localeCompare(c1.created)
            );
        } else {
    ...
    private handleAnnotationEvent(
        ...
        if (mutationType === MutationType.INITIAL) {
            ...
            this.docAnnotationIndex = DocAnnotationIndexes.delete(
                this.docAnnotationIndex,
                id
            );
