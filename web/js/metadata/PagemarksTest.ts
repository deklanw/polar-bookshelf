...
describe('Pagemarks', function() {
    describe('createRange', function() {
        it('for one page', function() {
            ...
            assertJSON(
                Object.values(DocMetas.getPageMeta(docMeta, 1).pagemarks),
                expected
            );
        });
        ...
        it('for two pages', function() {
            ...
            assertJSON(
                Object.values(DocMetas.getPageMeta(docMeta, 2).pagemarks),
                pagemark2
            );
        });
        ...
        it('for existing', function() {
            ...
            Pagemarks.updatePagemark(
                docMeta,
                3,
                Pagemarks.create({ percentage: 100 })
            );

            Pagemarks.updatePagemarksForRange(docMeta, 4);

            const assertPagemark = (pageNum: number) => {
                const pagemarks = Object.values(
                    DocMetas.getPageMeta(docMeta, pageNum).pagemarks
                );
        ...
        it('for fractional range', function() {
            ...
            Pagemarks.updatePagemark(
                docMeta,
                3,
                Pagemarks.create({ percentage: 50 })
            );

            Pagemarks.updatePagemarksForRange(docMeta, 4);

            const pagemarks = Object.values(
                DocMetas.getPageMeta(docMeta, 3).pagemarks
            );
