...
export class PagemarkRects {
     ...
     *
     */
    public static createDefault(pagemark: Pagemark) {
        if (
            pagemark.type === PagemarkType.SINGLE_COLUMN &&
            'percentage' in pagemark
        ) {
     ...
     * @return {PagemarkRect}
     */
    public static createFromPercentage(percentage: number) {
        return new PagemarkRect({
            left: 0,
            top: 0,
            width: 100,
     ...
     * @return {PagemarkRect}
     */
    public static createFromLines(xAxis: any, yAxis: any) {
     ...
     * @return {PagemarkRect}
     */
    public static createFromPositionedRect(boxRect: any, containerRect: any) {
        Preconditions.assertInstanceOf(boxRect, Rect, 'boxRect');

        const xAxis = boxRect
            .toLine('x')
            .multiply(100 / containerRect.width)
            .floor();
        const yAxis = boxRect
            .toLine('y')
            .multiply(100 / containerRect.height)
            .floor();
