 ...
 *
 */
export abstract class TypedPipe<E, M>
    implements ReadablePipe<E, M>, WritablePipe<M> {
    ...
    protected convertPipeNotification(
        pipeNotification: PipeNotification<any, any>
    ) {
        return new PipeNotification<E, M>(
            pipeNotification.channel,
            this.convertEvent(pipeNotification),
            this.convertMessage(pipeNotification.message)
        );
    }

    protected abstract convertEvent(
        pipeNotification: PipeNotification<any, any>
    ): E;
