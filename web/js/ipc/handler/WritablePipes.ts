...
export class WritablePipes {
    public static createFromContext<M>(
        targetContext: ElectronContext
    ): WritablePipe<M> {
        return WritablePipes.createFromFunction(
            (channel: string, message: M) => {
                // create a response pipe based on the context of the request.

                const sourceContext = ElectronContexts.create();

                switch (sourceContext.type) {
                    case ElectronContextType.MAIN:
                        WritablePipes.writeWithMainContext(
                            targetContext,
                            channel,
                            message
                        );
                    ...
                    case ElectronContextType.RENDERER:
                        WritablePipes.writeWithRendererContext(
                            targetContext,
                            channel,
                            message
                        );
    ...
    private static writeWithMainContext<M>(
        targetContext: ElectronContext,
        channel: string,
        message: M
    ) {
        ...
        switch (targetContext.type) {
            ...
            case ElectronContextType.RENDERER:
                const electronRendererContext = <ElectronRendererContext>(
                    targetContext
                );
                BrowserWindow.fromId(
                    electronRendererContext.windowReference.id
    ...
    private static writeWithRendererContext<M>(
        targetContext: ElectronContext,
        channel: string,
        message: M
    ) {
    ...
    public static createFromFunction<M>(
        writableFunction: WritableFunction<M>
    ): WritablePipe<M> {
        return new WritablePipeFunction(writableFunction);
    }
}

export type WritableFunction<M> = (channel: string, message: M) => void;
