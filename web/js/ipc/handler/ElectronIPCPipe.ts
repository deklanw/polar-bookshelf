...
export class ElectronIPCPipe extends IPCPipe<ElectronIPCEvent> {
    ...
    public convertEvent(
        pipeNotification: PipeNotification<Electron.Event, any>
    ): ElectronIPCEvent {
        const request = IPCMessage.create(pipeNotification.message);

        const responsePipe = WritablePipes.createFromFunction(
            (channel: string, message: IPCMessage<any>) => {
                // TODO migrate this to use: WritablePipes.createFromContext

                // create a response pipe based on the context of the request.

                const electronContext = ElectronContexts.create();

                if (
                    electronContext.type === ElectronContextType.RENDERER &&
                    ...
                    const target = <ElectronRendererContext>request.context;

                    ipcRenderer.sendTo(
                        target.windowReference.id,
                        channel,
                        message
                    );
                } else {
        ...
        const message = IPCMessage.create(pipeNotification.message);

        return new ElectronIPCEvent(
            responsePipe,
            message,
            pipeNotification.event.sender
        );
    }
}
