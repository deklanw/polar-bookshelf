...
const {
    DragRectAdjacencyCalculator,
} = require('../../pagemarks/controller/interact/drag/DragRectAdjacencyCalculator');
const {
    ResizeRectAdjacencyCalculator,
} = require('../../pagemarks/controller/interact/resize/ResizeRectAdjacencyCalculator');
...
export class BoxController {
    ...
    public register(opts: BoxOptions) {
        ...
        const restrictionElement = Optional.of(
            boxOptions.restrictionElement
        ...
        interact(boxOptions.target)
            ...
            .on('dragmove', (interactionEvent: any) => {
                ...
                const intersectedBoxes = this._calculateIntersectedBoxes(
                    interactionEvent.currentTarget,
                    Rects.createFromBasicRect({
                        left: origin.x,
                        top: origin.y,
                ...
                if (intersectedBoxes.intersectedRects.length === 0) {
                    ...
                    const intersectedRect =
                        intersectedBoxes.intersectedRects[0];

                    const adjacency = DragRectAdjacencyCalculator.calculate(
                        primaryRect,
                        intersectedRect,
                        restrictionRect
                    );
                    ...
                    if (adjustedRect) {
                        ...
                        // bug.
                        console.warn(
                            "Can't move due to no valid adjustedRect we can work with."
                        );
                    }
                }

                interactionEvent.interaction.lastBoxMoveEvent = this._fireBoxMoveEvent(
                    'drag',
                    restrictionRect,
                    boxRect,
                    target.id,
            ...
            .on('resizestart', (interactionEvent: any) => {
                this._captureStartTargetRect(interactionEvent);
                log.info(
                    'resizestart: interactionEvent.rect: ' +
                        JSON.stringify(interactionEvent.rect, null, '  ')
                );
                interactionEvent.interaction.startRect = Objects.duplicate(
            ...
            .on('resizemove', (interactionEvent: any) => {
                ...
                // JSON.stringify(interactionEvent.rect, null, "  "));
                log.info(
                    'resizemove: interactionEvent.interaction.startRect: ' +
                        JSON.stringify(
                            interactionEvent.interaction.startRect,
                            null,
                            '  '
                        )
                );
                ...
                const tempRect = Rects.createFromBasicRect(
                    interactionEvent.rect
                );

                const deltaRect = Rects.subtract(
                    tempRect,
                    interactionEvent.interaction.startRect
                );

                const resizeRect = Rects.add(
                    interactionEvent.interaction.startTargetRect,
                    deltaRect
                );
                ...
                log.info(
                    'resizemove: deltaRect: ' +
                        JSON.stringify(deltaRect, null, '  ')
                );
                ...
                if (intersectedBoxes.intersectedRects.length === 0) {
                    ...
                    const adjustedRect = resizeRectAdjacencyCalculator.calculate(
                        resizeRect,
                        intersectedRect,
                        rectEdges
                    );
                    ...
                    log.info(
                        'resizemove: adjustedRect: ' +
                            JSON.stringify(adjustedRect, null, '  ')
                    );
    ...
    public _fireCompletedBoxMoveEvent(interactionEvent: any) {
        if (interactionEvent.interaction.lastBoxMoveEvent) {
            const boxMoveEvent = Object.assign(
                {},
    ...
    public _calculateIntersectedBoxes(
        ...
        // log.info(`x: ${x}: y: ${y}`);
        log.info(
            '_calculateIntersectedBoxes: resizeRect is: ' +
                JSON.stringify(resizeRect, null, '  ')
        );
        ...
        const intersectedElements = element.parentElement!.querySelectorAll(
            intersectedElementsSelector
        );

        const boxes: HTMLElement[] = Array.from(intersectedElements)
        ...
        boxes.forEach(
            current => current.getAttribute('id') !== element.getAttribute('id')
        );
     ...
     * @private
     */
    public _computeOriginXY(interactionEvent: any) {
        const delta = {
            x:
                interactionEvent.pageX -
                interactionEvent.interaction.startCoords.page.x,
            y:
                interactionEvent.pageY -
                interactionEvent.interaction.startCoords.page.y,
        };
    ...
    public _moveTargetElement(x: number, y: number, target: HTMLElement) {
    ...
    public _resizeTargetElement(rect: Rect, target: HTMLElement) {
    ...
    public _captureStartTargetRect(interactionEvent: any) {
        // this modifies interactionEvent.interaction by side effect which I
        // don't like.
        interactionEvent.interaction.startTargetRect = Rects.fromElementStyle(
