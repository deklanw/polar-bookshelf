...
xdescribe('Create ranges', function() {
    xdescribe('with real data', function() {
        xit('my bitcoin book.', async function() {
            ...
            const fingerprint =
                '65393761393531623135393737626562666234373866653365396535313036623631346666376461623662383239616439666637353064393132643133353030';

            const docMeta = await persistenceLayer.getDocMeta(fingerprint);

            assert.ok(docMeta !== undefined);

            const createPagemarksForPageRanges = new CreatePagemarksForPageRanges(
                docMeta!
            );

            createPagemarksForPageRanges.execute({
                range: { start: 1, end: 204 },
            });
