...
export class ResourcePaths {
    ...
    public static absoluteFromRelativePath(relativePath: string) {
        ...
        throw new Error(
            `No file found for ${relativePath} in baseDirs: ` +
     ...
     * @param relativeURL
     */
    public static resourceURLFromRelativeURL(
        relativeURL: string,
        useFileURL: boolean = USE_FILE_URL
    ): string {
        ...
        if (useFileURL) {
            const absolutePath = ResourcePaths.absoluteFromRelativePath(
                relativePath
            );
