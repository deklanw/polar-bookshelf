...
describe('Objects', function() {
    describe('defaults', function() {
        it('with no current', function() {
            const value = Objects.defaults(null, { hello: 'world' });

            assertJSON(value, { hello: 'world' });
        });

        it('with no value', function() {
            const value = Objects.defaults({}, { hello: 'world' });

            assertJSON(value, { hello: 'world' });
        });

        it('with existing', function() {
            const value = Objects.defaults(
                { hello: 'buddy' },
                { hello: 'world' }
            );
    ...
    describe('createInstance', function() {
        ...
        it('should not compile', function() {
            ...
            function createInstance<T>(prototype: T, val: any) {
                const result: T = create(prototype);
                return val;
            }

            const val: string = createInstance(Address.prototype, {});
        });
    });
});
