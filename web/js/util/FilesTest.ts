...
describe('Files', function() {
    describe('createDirectorySnapshot', function() {
        ...
        xit('Test with non-existant directory', async function() {
            ...
            const targetPath = FilePaths.join(
                tmpdir,
                'non-existent-directory-path.new'
            );
        ...
        it('Test with empty dir', async function() {
            ...
            const result = await Files.createDirectorySnapshot(
                path,
                targetPath
            );
            ...
            assert.ok(
                await Files.existsAsync(targetPath),
                'no target path: ' + targetPath
            );
        });
        ...
        it('Test with one file', async function() {
            ...
            assert.ok(
                await Files.existsAsync(FilePaths.join(targetPath, 'hello.txt'))
            );
        });
        ...
        it('Test with nested dirs', async function() {
            ...
            await Files.writeFileAsync(
                FilePaths.join(path, 'dir0', 'hello.txt'),
                'hello'
            );
            ...
            assert.ok(
                await Files.existsAsync(
                    FilePaths.join(targetPath, 'dir0', 'hello.txt')
                )
            );
        });
    });
    ...
    describe('writeFileAsync', function() {
        ...
        it('basic', async function() {
            await Files.writeFileAsync(
                FilePaths.join(tmpdir, 'write-file-async.txt'),
                'hello world'
            );
        });
        ...
        it('from stream', async function() {
            ...
            await Files.writeFileAsync(
                dataOutputPath,
                Files.createReadStream(dataInputPath)
            );
    ...
    describe('statAsync', function() {
        ...
        xit('missing file', async function() {
            assert.throw(async function() {
                await Files.statAsync(
                    FilePaths.createTempName('invalid-file-name')
                );
            });
        });
    });
    ...
    describe('mkdirAsync', function() {
        ...
        xit('nested', async function() {
            ...
            const nestedDirPath = FilePaths.join(
                tmpdir,
                'base-of-nested-dirs',
                'first',
                'second',
