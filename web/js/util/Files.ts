...
export class Files {
     ...
     *
     */
    public static async createDirectorySnapshot(
        path: string,
        targetPath: string,
        filter: DirectorySnapshotPredicate = ACCEPT_ALL
        ...
        for (const dirEntry of dirEntries) {
            ...
            if (dirEntryType === 'directory') {
                const dirResult = await this.createDirectorySnapshot(
                    dirEntryPath,
                    targetFilePath,
                    filter
                );
                ...
                throw new Error(
                    `Unable to handle dir entry: ${dirEntryPath} of type ${dirEntryType}`
                );
            }
        }
    ...
    public static async removeDirectoryRecursivelyAsync(
        path: string
        ...
        for (const dirEntry of dirEntries) {
            ...
            if (dirEntryType === 'directory') {
                const dirResult = await this.removeDirectoryRecursivelyAsync(
                    dirEntryPath
                );
                dirs.push(dirResult);
            } else if (dirEntryType === 'file') {
     ...
     * @param mode
     */
    public static async createDirAsync(
        dir: string,
        mode?: number | string | undefined | null
    ) {
        const result: CreateDirResult = {
    ...
    public static async readFileAsync(
        ...
        return this.withProperException(() =>
            this.promised.readFileAsync(path)
        );
    }
    ...
    public static async writeFileAsync(
        ...
        if (data instanceof Buffer || typeof data === 'string') {
            return this.withProperException(() =>
                this.promised.writeFileAsync(path, data, options)
            );
        } else if (FileHandles.isFileHandle(data)) {
            const fileRef = <FileHandle>data;
            Files.createReadStream(fileRef.path).pipe(
    ...
    public static async mkdirAsync(
        path: string,
        mode?: number | string | undefined | null
    ): Promise<void> {
        return this.withProperException(() =>
            this.promised.mkdirAsync(path, mode)
        );
    }

    public static async accessAsync(
        path: PathLike,
        mode: number | undefined
    ): Promise<void> {
        return this.withProperException(() =>
            this.promised.accessAsync(path, mode)
        );
    }
    ...
    public static async linkAsync(
        existingPath: PathLike,
        newPath: PathLike
    ): Promise<void> {
        return this.withProperException(() =>
            this.promised.linkAsync(existingPath, newPath)
        );
    }
    ...
    public static async realpathAsync(path: string): Promise<string> {
        return this.withProperException(() =>
            this.promised.realpathAsync(path)
        );
    }
    ...
    public static async copyFileAsync(
        ...
        return this.withProperException(() =>
            this.promised.copyFileAsync(src, dest, flags)
        );
    }

    public static async appendFileAsync(
        path: string | Buffer | number,
        data: string | Buffer,
        options?: AppendFileOptions
    ): Promise<void> {
        return this.withProperException(() =>
            this.promised.appendFileAsync(path, data, options)
        );
    }
    ...
    public static async openAsync(
        ...
        return this.withProperException(() =>
            this.promised.openAsync(path, flags, mode)
        );
    }
    ...
    private static async withProperException<T>(
        func: () => Promise<T>
    ): Promise<T> {
...
export type DirectorySnapshotPredicate = (
    path: string,
    targetPath: string
) => boolean;
