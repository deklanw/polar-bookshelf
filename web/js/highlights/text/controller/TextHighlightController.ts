...
import {
    TextHighlightRecord,
    TextHighlightRecords,
} from '../../../metadata/TextHighlightRecords';
...
export class TextHighlightController {
    constructor(model: Model) {
        ...
        if (ipcRenderer) {
            ...
            ipcRenderer.on(
                ...
                (event: Electron.EventEmitter, arg: any) => {
                    switch (arg.command) {
    ...
    public onDocumentLoaded() {
        log.debug(
            'TextHighlightController.onDocumentLoaded: ',
            this.model.docMeta
        );
    }
    ...
    private onMessageReceived(event: any) {
        ...
        switch (event.data.type) {
            case 'create-text-highlight':
                const typedMessage: TypedMessage<HighlightCreatedEvent> =
                    event.data;

                this.doHighlight(typedMessage.value.highlightColor).catch(err =>
                    log.error('Unable to create text highlight', err)
                );
    ...
    public doHighlightLegacy(highlightColor: HighlightColor) {
        const textHighlighter = this.createLegacyTextHighlighter(
            highlightColor
        );
    ...
    public createLegacyTextHighlighter(highlightColor: HighlightColor) {
            ...
            onAfterHighlight: (range: any, highlightElements: any) => {
                ...
                highlightElements.forEach(function(highlightElement: any) {
                    ...
                    highlightElement.className =
                        highlightElement.className + ' ' + highlightClazz;
                });

                (async () => {
                    await controller.onTextHighlightCreatedLegacy(
                        '.' + highlightClazz,
                        highlightColor
                    );
        ...
        textHighlighter = TextHighlighterFactory.newInstance(
            targetDocument!.body,
            textHighlighterOptions
        );
     ...
     */
    private async onTextHighlightCreatedLegacy(
        selector: string,
        ...
        await this.createTextHighlight(async () => {
            ...
            const textHighlightRows: TextHighlightRow[] = TextHighlightRows.createFromSelector(
                selector
            );
            ...
            const textSelections = TextExtracter.toTextSelections(
                textHighlightRows
            );

            return TextHighlightRecords.create(
                rects,
                textSelections,
                { TEXT: text },
                highlightColor
    ...
    public async createTextHighlight(
        factory: () => Promise<TextHighlightRecord>
        ...
        const screenshotImageRef = this.toImage(
            screenshotID,
            'screenshot',
            screenshotDimensions
        );

        TextHighlights.attachImage(
            textHighlightRecord.value,
            screenshotImageRef
        );
        ...
        const pageMeta = this.model.docMeta.getPageMeta(
            currentPageMeta.pageNum
        );
        ...
        pageMeta.textHighlights[textHighlightRecord.id] =
            textHighlightRecord.value;

        const capturedScreenshot = await selectionScreenshot.capturedScreenshotPromise;

        const screenshot = this.toScreenshot(
            screenshotID,
    ...
    private onTextHighlightDeleted(triggerEvent: TriggerEvent) {
        ...
        const annotationPointers = AnnotationPointers.toAnnotationPointers(
            '.text-highlight',
            triggerEvent
        );
        ...
        Optional.first(...annotationPointers).map(annotationDescriptor => {
            log.info(
                'Deleting annotationDescriptor: ',
                JSON.stringify(annotationDescriptor, null, '  ')
            );
            ...
            const textHighlight =
                pageMeta.textHighlights[annotationDescriptor.id];
