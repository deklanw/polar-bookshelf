...
export class TextHighlightComponent extends Component {
    ...
    public init(annotationEvent: AnnotationEvent) {
        ...
        this.pageElement = <HTMLElement>(
            this.docFormat.getPageElementFromPageNum(this.pageNum)
        );
    }
    ...
    public render() {
        ...
        Dictionaries.forDict<Rect>(
            this.textHighlight!.rects,
            (id, highlightRect) => {
                const pageElement = Preconditions.assertPresent(
                    this.pageElement
                );
                const pageMeta = Preconditions.assertPresent(this.pageMeta);
                const docMeta = Preconditions.assertPresent(this.docMeta);
                const textHighlight = Preconditions.assertPresent(
                    this.textHighlight
                );

                log.debug(
                    'Rendering annotation at: ' +
                        JSON.stringify(highlightRect, null, '  ')
                );
                ...
                highlightElement.className = `text-highlight annotation text-highlight-${
                    textHighlight.id
                }`;
                ...
                pageElement.insertBefore(
                    highlightElement,
                    pageElement.firstChild
                );
            }
        );
    }
    ...
    public destroy() {
        ...
        log.debug(
            `Found N elements for selector ${selector}: ` +
