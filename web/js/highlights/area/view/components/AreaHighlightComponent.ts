...
export class AreaHighlightComponent extends Component {
    ...
    public init(annotationEvent: AnnotationEvent) {
        ...
        this.boxController = new BoxController(boxMoveEvent =>
    ...
    public onBoxMoved(boxMoveEvent: BoxMoveEvent) {
        ...
        if (boxMoveEvent.state === 'completed') {
            ...
            log.debug(
                'New areaHighlight: ',
                JSON.stringify(this.areaHighlight, null, '  ')
            );

            delete annotationEvent.pageMeta.areaHighlights[
                this.areaHighlight.id
            ];
            annotationEvent.pageMeta.areaHighlights[
                this.areaHighlight.id
            ] = this.areaHighlight;
        } else {
        }
    ...
    public render() {
        ...
        const pageElement = this.docFormat.getPageElementFromPageNum(
            pageMeta.pageInfo.num
        );
        const dimensionsElement = pageElement.querySelector(
        ...
        forDict(areaHighlight.rects, (key, rect) => {
            ...
            log.debug(
                'Rendering annotation at: ' +
                    JSON.stringify(overlayRect, null, '  ')
            );
            ...
            if (highlightElement === null) {
                ...
                log.debug(
                    'Creating box controller for highlightElement: ',
                    highlightElement
                );

                boxController.register(
                    new BoxOptions({
            ...
            highlightElement.className = `area-highlight annotation area-highlight-${
                areaHighlight.id
            }`;
    ...
    public destroy() {
        ...
        log.debug(
            `Found N elements for selector ${selector}: ` + elements.length
        );
