...
const {
    TextHighlightRows,
} = require('./highlights/text/controller/TextHighlightRows');
...
const date = new Date(Date.parse('2018-05-30T02:47:44.411Z'));
...
describe('Test computeRectForRow for highlighting text...', function() {
    it('Test with 1 entries', function() {
        ...
        assert.deepEqual(
            TextHighlightRows.computeRectForRow(rectElements),
            expected
        );
    });
...
describe('Testing computeContiguousRects.', function() {
    ...
    xit('Test with one entry', function() {
        ...
        const actual = TextHighlightRows.computeContiguousRects(rects);
    ...
    xit('Test with two entries', function() {
        ...
        assertJSON(
            TextHighlightRows.computeContiguousRects(boundingClientRects),
            expected
        );
    });
});
...
describe('Testing createSiblingTupples.', function() {
    ...
    it('Test with 1 entry', function() {
        assert.deepEqual(utils.createSiblingTuples([1]), [
...
describe('Testing bounding client rect utils.', function() {
    it('Test with one element', function() {
        ...
        const cbr = utils.getBoundingClientRectFromBCRs(boundingClientRects);
...
describe('Testing Delegates', function() {
    it('Test basic functionality', function() {
        class MyDelegate {
            public testArg0: string | null = null;
            public testArg1: string | null = null;
            ...
            public testFunction(testArg0: string, testArg1: string) {
                this.testArg0 = testArg0;
                this.testArg1 = testArg1;
            }
        }

        const delegate0 = new MyDelegate();
        const delegate1 = new MyDelegate();

        const delegator = new utils.Delegator([delegate0, delegate1]);
