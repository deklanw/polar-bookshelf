...
export class PendingWebRequestsListener extends BaseWebRequestsListener {
    ...
    public processWebRequestEvent(event: INamedWebRequestEvent) {
        ...
        if (pendingChange) {
            log.info(
                `Pending state ${pendingChange} for request id=${
                    details.id
                } to ${pending} on ${name} for URL: ${details.url}`
            );
        }
        ...
        if (pending < 5) {
            log.debug(
                'The following pending requests remain: \n',
...
export type PendingWebRequestsCallback = (
    event: PendingWebRequestsEvent
) => void;
