 ...
 */
export class CreateAnnotationHandler extends IPCHandler<
...
> {
    ...
    protected async handleIPC(
        event: IPCEvent,
        annotationContainer: AnnotationContainer<Annotation>
    ): Promise<any> {
        ...
        if (descriptor.type === AnnotationType.FLASHCARD) {
            const flashcard = new Flashcard(<Flashcard>(
                annotationContainer.value
            ));

            if (
                descriptor.docFingerprint ===
                ...
                log.info(
                    `Ignoring flashcard.  ${descriptor.docFingerprint} != ${
                        this.model.docMeta.docInfo.fingerprint
                    }`
                );
            }
