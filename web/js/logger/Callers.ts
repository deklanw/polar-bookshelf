export class Callers {
    ...
    public static _parse(frame: string) {
        ...
        const javascriptCaller = Callers.parseRE(
            frame,
            /([^/.\\)]+\.(js|ts|tsx|jsx)):[0-9]+:[0-9]+\)?$/g
        );

        // this returns the first match with a space at the end.
        const webpackCaller = Callers.parseRE(
            frame,
            /([^/.\\)]+\.(js|ts|tsx|jsx))( |\?)/g
        );
    ...
    public static parseRE(frame: string, re: RegExp): Caller | undefined {
