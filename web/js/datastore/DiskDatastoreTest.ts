...
describe('DiskDatastore', async function() {
    ...
    it('getDataDirsForPlatform MAC_OS', function() {
        ...
        assertJSON(
            DiskDatastore.getDataDirsForPlatform({ userHome, platform }),
            {
                paths: [
                    '/Users/alice/.polar',
    ...
    it('init and test paths', async function() {
        ...
        assert.equal(
            diskDatastore.dataDir,
            FilePaths.join(tmpdir, 'test-paths')
        );

        assert.equal(
            diskDatastore.stashDir,
            FilePaths.join(tmpdir, 'test-paths', 'stash')
        );
    ...
    it('Add file and remove file from the stash and see if it exists.', async function() {
        const path = await Files.realpathAsync(
            FilePaths.join(__dirname, '..', '..', '..', 'docs', 'example.pdf')
        );

        assert.ok(
            await Files.existsAsync(path),
            'No file found from: ' + process.cwd()
        );
        ...
        await diskDatastore.writeFile(
            Backend.STASH,
            { name: 'example.pdf' },
            await Files.readFileAsync(path)
        );

        const pdfPath = FilePaths.join(dataDir, 'stash', 'example.pdf');

        assert.ok(
            await Files.existsAsync(pdfPath),
            'Could not find file: ' + pdfPath
        );

        assert.ok(
            await diskDatastore.containsFile(Backend.STASH, {
                name: 'example.pdf',
            })
        );
    ...
    it('Delete file and make sure state.json and dir are no longer present', async function() {
        ...
        await MockPHZWriter.write(
            FilePaths.create(diskDatastore.stashDir, `${fingerprint}.phz`)
        );
