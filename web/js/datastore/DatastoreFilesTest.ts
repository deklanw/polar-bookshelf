...
describe('DastastoreFiles', function() {
    ...
    it('sanitizeFilename', async function() {
        assert.equal(
            DatastoreFiles.sanitizeFileName('asdf/ \\ : * ? " < > |asdf'),
            'asdf_ _ _ _ _ _ _ _ _asdf'
        );

        assert.equal(
            DatastoreFiles.isSanitizedFileName('asdf/ \\ : * ? " < > |asdf'),
            false
        );
        assert.equal(
            DatastoreFiles.isSanitizedFileName('asdf_ _ _ _ _ _ _ _ _asdf'),
            true
        );
    });
});
