...
export class PersistenceLayers {
    ...
    public static async toSyncDocMapFromDocs(
        datastore: Datastore,
        ...
        for (const docMetaRef of docMetaRefs) {
            work.push(async () => {
                ...
                if (isPresent(docMeta)) {
                    syncDocsMap[docMetaRef.fingerprint] = SyncDocs.fromDocInfo(
                        docMeta!.docInfo,
                        'created'
                    );
     ...
     */
    public static async synchronizeOrigins(
        localSyncOrigin: SyncOrigin,
        cloudSyncOrigin: SyncOrigin,
        listener: DocMetaSnapshotEventListener = ASYNC_NULL_FUNCTION
    ): Promise<void> {
        log.notice('Transferring from local -> cloud...');
        const localToCloud = await PersistenceLayers.transfer(
            localSyncOrigin,
            cloudSyncOrigin,
            listener,
            'local-to-cloud'
        ...
        const cloudToLocal = await PersistenceLayers.transfer(
            cloudSyncOrigin,
            localSyncOrigin,
            listener,
            'cloud-to-local'
    ...
    public static async transfer(
        ...
        async function handleSyncFile(syncDoc: SyncDoc, fileRef: FileRef) {
            ...
            if (
                !(await target.datastore.containsFile(Backend.STASH, fileRef))
            ) {
                let optionalFile: Optional<DatastoreFile>;

                try {
                    optionalFile = await source.datastore.getFile(
                        Backend.STASH,
                        fileRef
                    );
                } catch (e) {
                    console.error(
                        `Could not get file ${
                            fileRef.name
                        } for doc with fingerprint: ${syncDoc.fingerprint}`,
                        fileRef
                    );
                    throw e;
                }
                ...
                if (optionalFile.isPresent()) {
                    ...
                    await target.datastore.writeFile(
                        file.backend,
                        fileRef,
                        buffer,
                        file.meta
        ...
        async function handleSyncDoc(
            sourceSyncDoc: SyncDoc,
            targetSyncDoc?: SyncDoc
        ) {
            ...
            if (doWriteDocMeta) {
                const data = await source.datastore.getDocMeta(
                    sourceSyncDoc.fingerprint
                );
                await target.datastore.write(
                    sourceSyncDoc.fingerprint,
                    data,
                    sourceSyncDoc.docMetaFileRef.docInfo
                );
        ...
        const progressID = `transfer:source=${source.datastore.id},target=${
            target.datastore.id
        }`;

        const progressTracker = new ProgressTracker(
            sourceSyncDocs.length,
            progressID
        );
        ...
        for (const sourceSyncDoc of sourceSyncDocs) {
            ...
            const handler = async () => {
                try {
                    ...
                    log.error('Unable to sync between source and target: ', {
                        sourceSyncDoc,
                        targetSyncDoc,
                    });
                }
