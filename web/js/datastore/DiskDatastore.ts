...
export class DiskDatastore extends AbstractDatastore implements Datastore {
    ...
    public async delete(
        ...
        if (docMetaFileRef.docFile && docMetaFileRef.docFile.name) {
            ...
            docPath = FilePaths.join(
                this.directories.stashDir,
                docMetaFileRef.docFile.name
            );
        }
        ...
        const deleteDocPathPromise =
            docPath !== undefined
                ? Files.deleteAsync(docPath)
                : Promise.resolve(undefined);

        // now handle all the promises with the datastore mutation so that we
        // can verify that we were written and committed.
        this.datastoreMutations.handle(
            Promise.all([deleteStatePathPromise, deleteDocPathPromise]),
            datastoreMutation,
            () => true
        );
    ...
    public async getDocMeta(fingerprint: string): Promise<string | null> {
        ...
        const canAccess = await Files.accessAsync(
            statePath,
    ...
    public async getFile(
        ...
        if (await Files.existsAsync(fileReference.path)) {
            const datastoreFile = await this.createDatastoreFile(
                backend,
                ref,
                fileReference
            );
    ...
    public async write(
        ...
        const result = Files.writeFileAsync(statePath, data, {
            encoding: 'utf8',
        });
    ...
    public async getDocMetaFiles(): Promise<DocMetaRef[]> {
        ...
        for (const entry of entries) {
            ...
            if (docMetaDirStat.isDirectory()) {
                const stateFile = FilePaths.join(
                    this.dataDir,
                    entry,
                    'state.json'
                );
    ...
    public async snapshot(
        ...
        return Datastores.createCommittedSnapshot(
            this,
            docMetaSnapshotEventListener
        );
    }
    ...
    public async createBackup(): Promise<void> {
        ...
        await Files.createDirectorySnapshot(
            dataDir,
            backupDir,
            acceptPredicate
        );
    ...
    private async createDatastoreFile(
        backend: Backend,
        ref: FileRef,
        fileReference: DiskFileReference
    ): Promise<DatastoreFile> {
    ...
    public static async determineProperDirectory(
    ...
    public static getDataDirsForPlatform(dirRuntime: DirRuntime): DirectorySet {
        ...
        switch (platform) {
            case Platform.WINDOWS: {
                ...
                return {
                    paths: [FilePaths.join(userHome, '.polar'), preferredPath],
                    preferredPath,
                };
            }

            case Platform.LINUX: {
                const preferredPath = FilePaths.join(
                    userHome,
                    '.config',
                    'Polar'
                );
                ...
                return {
                    paths: [FilePaths.join(userHome, '.polar')],
                    preferredPath,
                };
            }
