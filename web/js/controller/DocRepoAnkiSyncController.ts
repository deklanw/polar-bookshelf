...
export class DocRepoAnkiSyncController {
    ...
    constructor(
        ...
        syncBarProgress: IEventDispatcher<SyncBarProgress>
    ) {
    ...
    private async onStartSync() {
        ...
        const docMetaSuppliers: DocMetaSupplierCollection = docMetaFiles.map(
            docMetaFile => {
                return async () => {
                    log.info(
                        'Reading docMeta for anki sync: ' +
                            docMetaFile.fingerprint
                    );
                    return (await persistenceLayer.getDocMeta(
                        docMetaFile.fingerprint
                    ))!;
                };
            }
        ...
        const pendingSyncJob = await ankiSyncEngine.sync(
            docMetaSuppliers,
