...
export class WebController extends Controller {
    ...
    public async dispatchDocumentLoaded(
        fingerprint: string,
        nrPages: number,
    ...
    public setupWindowWidth() {
        const viewerClientWidth = Optional.of(
            document.querySelector('#viewerContainer')
        )
            .map(current => current.clientWidth)
            .getOrElse(0);

        const viewerScrollWidth = Optional.of(
            document.querySelector('#viewerContainer')
        )
        ...
        if (needsResize) {
            const sidebarScrollWidth = Optional.of(
            ...
            const newWidth =
                sidebarScrollWidth + viewerScrollWidth + bufferWidth;
    ...
    public setupDocumentTitle() {
        const title = Optional.of(this.model.docMeta.docInfo.title).getOrElse(
            'Untitled'
        );
    ...
    public listenForDocumentLoad() {
        ...
        container.addEventListener(
            'pagesinit',
            this.detectDocumentLoadedEventListener.bind(this)
        );
        container.addEventListener(
            'updateviewarea',
            this.detectDocumentLoadedEventListener.bind(this)
    ...
    public detectDocumentLoadedEventListener(event: Event) {
        ...
        if (
            currentDocFingerprint !== undefined &&
            currentDocFingerprint !== this.docFingerprint
        ) {
            ...
            this.onNewDocumentFingerprint(
                newDocumentFingerprint,
                currentDocState.nrPages,
                currentDocState.currentPageNumber
            );
        }
    }

    public onNewDocumentFingerprint(
        newDocumentFingerprint: string,
        nrPages: number,
        currentPageNumber: number
    ) {
        log.info(
            `Detected new document fingerprint (fingerprint=${newDocumentFingerprint}, nrPages=${nrPages}, currentPageNumber=${currentPageNumber})`
        );

        this.docFingerprint = newDocumentFingerprint;

        this.dispatchDocumentLoaded(
            newDocumentFingerprint,
            nrPages,
            currentPageNumber
    ...
    public async keyBindingPagemarkEntirePage(event: KeyboardEvent) {
        ...
        if (pageElement) {
            const pageNum = this.docFormat.getPageNumFromPageElement(
                pageElement
            );
    ...
    public async listenForKeyBindings() {
        document.addEventListener(
            'keydown',
            this.keyBindingListener.bind(this)
        );
