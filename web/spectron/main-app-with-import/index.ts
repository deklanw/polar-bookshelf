...
SpectronMain2.create({ windowFactory: createWindow }).run(async state => {
    ...
    const pdfStashPath = FilePaths.join(
        PolarDataDir.get()!,
        'stash',
        '12i77BKrNy-example.pdf'
    );

    log.info('Testing for file: ' + pdfStashPath);
    assert.ok(
        await Files.existsAsync(pdfStashPath),
        'File does not exist: ' + pdfStashPath
    );
