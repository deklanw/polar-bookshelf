...
import {
    FirebaseDatastore,
    DatastoreCollection,
} from '../../js/datastore/FirebaseDatastore';
...
function renderJSON(name: string, object: any) {
    ...
    ReactDOM.render(
        <ReactJson src={object} name={name} shouldCollapse={() => true} />,
        objectElementHolder
    );
...
SpectronRenderer.run(async state => {
    new FirebaseRunner(state).run(async () => {
        const onSnapshot = (
            ...
            const snapshotFacade = SnapshotFacades.toSnapshotFacade(
                collection,
                snapshot
            );
...
export class SnapshotFacades {
    public static toSnapshotFacade(
        collection: string,
        snapshot: firebase.firestore.QuerySnapshot
    ): SnapshotFacade {
        const docChanges: DocumentChangeFacade[] = snapshot
            .docChanges()
            .map(current => {
                return {
                    type: current.type,
        ...
        const docs: DocFacade[] = snapshot.docs.map(current => {
            return {
                data: current.data(),
            };
        });
