...
async function createDatastore() {
    ...
    const cloudAwareDatastore = new CloudAwareDatastore(
        diskDatastore,
        firebaseDatastore
    );

    cloudAwareDatastore.shutdownHook = async () => {
        const consistency = await Datastores.checkConsistency(
            diskDatastore,
            firebaseDatastore
        );
        ...
        if (!consistency.consistent) {
            console.log(
                'Filesystems are NOT consistent: ',
                consistency.manifest0,
                consistency.manifest1
            );
        }
...
SpectronRenderer.run(async state => {
    new FirebaseRunner(state).run(async () => {
        await PolarDataDir.useFreshDirectory(
            '.test-firebase-cloud-aware-datastore'
        );
        ...
        describe('Cloud datastore tests', function() {
            beforeEach(async function() {
                try {
                    ...
                    await Files.removeDirectoryRecursivelyAsync(
                        PolarDataDir.get()!
                    );
            ...
            type ConsistencyTestFunction = (
                persistenceLayer: PersistenceLayer,
                cloudAwareDatastore: CloudAwareDatastore
            ) => Promise<void>;
             ...
             * datastore is valid.
             */
            async function testForConsistency(
                testFunction: ConsistencyTestFunction = ASYNC_NULL_FUNCTION
            ) {
                const cloudAwareDatastore = await createDatastore();
                const persistenceLayer = new DefaultPersistenceLayer(
                    cloudAwareDatastore
                );
            ...
            it('Test8: Sync with extra files in the local store', async function() {
                ...
                await datastore.writeDocMeta(
                    MockDocMetas.createMockDocMeta('0x0004')
                );
                await datastore.writeDocMeta(
                    MockDocMetas.createMockDocMeta('0x0005')
                );
                await datastore.stop();

                await testForConsistency(
                    async (persistenceLayer, cloudAwareDatastore) => {
                        for (const currentFingerprint of ['0x0004', '0x0005']) {
                            assert.ok(
                                await cloudAwareDatastore.local.contains(
                                    currentFingerprint
                                )
                            );
                            assert.ok(
                                await cloudAwareDatastore.cloud.contains(
                                    currentFingerprint
                                )
                            );
                        }
            ...
            it('Test2: Basic synchronization tests', async function() {
                ...
                const firestorePersistenceLayer = new DefaultPersistenceLayer(
                    firebaseDatastore
                );

                await firestorePersistenceLayer.writeDocMeta(
                    MockDocMetas.createMockDocMeta('0x001')
                );
                ...
                cloudAwareDatastore.addDocMetaSnapshotEventListener(
                ...
                await firestorePersistenceLayer.writeDocMeta(
                    MockDocMetas.createMockDocMeta('0x002')
                );
                ...
                await waitForExpect(async () => {
                    assert.ok(
                        await persistenceLayer.contains('0x002'),
                        'Does not contain second doc'
                    );
                });
            ...
            it('Test3: Write a basic doc with synchronization listener', async function() {
                ...
                cloudAwareDatastore.addSynchronizationEventListener(
                ...
                const firebasePersistenceLayer = new DefaultPersistenceLayer(
                    new FirebaseDatastore()
                );
                await firebasePersistenceLayer.init();
                await firebasePersistenceLayer.writeDocMeta(
                    MockDocMetas.createMockDocMeta('0x003')
                );
            ...
            it('Test4: Write a basic doc', async function() {
                ...
                datastoreMutation.written
                    .get()
                    .then(() => (writtenDuration = Date.now() - before));
                datastoreMutation.committed
                    .get()
                    .then(() => (committedDuration = Date.now() - before));
                ...
                console.log(
                    `writtenDuration: ${writtenDuration}, committedDuration: ${committedDuration}`
                );
            ...
            it('Test5: Test an existing firebase store with existing data replicating to a new CloudDatastore.', async function() {
                ...
                const sourcePersistenceLayer = new DefaultPersistenceLayer(
                    new FirebaseDatastore()
                );
                await sourcePersistenceLayer.init(errorListener);
                await sourcePersistenceLayer.writeDocMeta(
                    MockDocMetas.createMockDocMeta(fingerprint)
                );
                await sourcePersistenceLayer.stop();

                const targetPersistenceLayer = new DefaultPersistenceLayer(
                    await createDatastore()
                );
            ...
            it('Test6: Verify unsubscribe works.', async function() {
                ...
                const snapshotResult = await targetPersistenceLayer.snapshot(
                    async event => {
                        console.log(
                            'GOT AN EVENT with consistency: ' +
                                event.consistency,
                            event
                        );
                ...
                const sidePersistenceLayer = new DefaultPersistenceLayer(
                    new FirebaseDatastore()
                );
                await sidePersistenceLayer.init();
                await sidePersistenceLayer.writeDocMeta(
                    MockDocMetas.createMockDocMeta()
                );
                ...
                assert.ok(
                    gotEventAfterUnsubscribe === false,
                    'Nope.. we still got the event'
                );
            });
            ...
            xit('Test7: Test a remote write and a local replication to disk', async function() {
                ...
                await waitForExpect(async () => {
                    ...
                    assert.ok(
                        await Files.existsAsync(path),
                        'Path for fingerprint never appeared'
                    );
                });
