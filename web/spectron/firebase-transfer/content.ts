    ...
    new FirebaseRunner(state).run(async () => {
        ...
        async function initialMergeWithFirebase() {
            ...
            async function toSyncDocMap(persistenceLayer: PersistenceLayer) {
                const timeLabel =
                    'toSyncOrigin:' + persistenceLayer.datastore.id;

                try {
                    console.time(timeLabel);
                    return await PersistenceLayers.toSyncDocMap(
                        persistenceLayer.datastore,
                        progressState => {
            ...
            await PersistenceLayers.merge(
                await toSyncOrigin(source),
                await toSyncOrigin(target),
                async transferEvent => {
